...
class record
{
    ...
    public function search($query=null,$template=null,$dateStart=null,$dateEnd=null)
    {
        ...
        $sql = "SELECT tr.id, tr.car_info_id, tr.status_id, tr.car_number, tr.owner_surname, tr.date_created,
            ...
            INNER JOIN car_info ci ON tr.car_info_id = ci.id";
        if($query){
            $sql=$sql." WHERE car_number LIKE '%" . $query . "%' OR owner_surname LIKE '%" . $query . "%'";
        }
        elseif($template){
            $sql=$sql." WHERE car_number REGEXP '" . $template . "' OR owner_surname REGEXP '" . $template . "'";
        }
        elseif ($dateStart&&$dateEnd){
            $sql=$sql." WHERE tr.date_created between '" . $dateStart . "' and '". $dateEnd . "'";
        }
        $stmt = $conn->prepare($sql);
        $stmt->execute();
